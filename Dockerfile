...
RUN apt-get update \
    ...
    && git clone https://github.com/raspberrypi/pico-sdk.git --no-check-certificate\
    && cd pico-sdk\
    && git submodule update --init\
    && cd ../ \
    && mv pico-sdk /opt/pico-sdk \
    && git clone https://github.com/raspberrypi/pico-examples.git --no-check-certificate\
    && git clone https://github.com/raspberrypi/picotool.git --no-check-certificate\
    && cd picotool \
    && mkdir build \
    && cd build \
    && cmake .. \
